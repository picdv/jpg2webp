...
# 压缩图片的key
online_key_list = str(sys.argv[1]).split(',')
# 获取key
online_key_list_iter = iter(online_key_list)
online_key = next(online_key_list_iter)

# 需要压缩图片的路径
fromPath = "/home/runner/work/jpg2webp/jpg2webp/content/raws"  # source dir path
# 压缩后下载的图片路径
...
def compress_online(sourcefile, outputfile):
    ...
    try:
        ...
        source = tinifyAPi.from_file(sourcefile).convert(type="image/webp")
    ...
    except tinify.ClientError as ClientError:
        # Check your source image and request options.
        print("Check your source image and request options.", ClientError)
        rs = False
        pass
    except tinify.ServerError as se:
        # Temporary issue with the Tinify API.
        # print("Temporary issue with the Tinify API. %s" % e.message)
        print("Temporary issue with the Tinify API.", se)

        rs = False
        pass
    except tinify.ConnectionError as ce:
        # A network connection error occurred.
        print("网络故障。。。休息1秒继续", ce)
        time.sleep(1)
        compress_online(sourcefile, outputfile)  # 递归方法 继续读取
        rs = True
        pass
        ...
        print("Something else went wrong, unrelated to the Tinify API.", e)
        rs = False
        pass
...
for root, dirs, files in os.walk(fromPath):
    ...
    for name in files:
        new_name = name.split('.')[0]+'.webp'
        newFromFilePath = os.path.join(root, name)
        newToFilePath = os.path.join(newToPath, new_name)
